...
import { Box, Drawer, List, Divider, ListItem, ListItemButton, ListItemText, IconButton, Stack,  Typography, Avatar, Chip ,Button} from '@mui/material';
    ...
    <>
      <Box onClick={toggleDrawer(true)} sx={{ position: 'relative', width: 44, height: 44, cursor: 'pointer', }}>
        <Box sx={{ position: 'absolute', inset: 0, borderRadius: '50%', padding: '2px', background: (theme) => `conic-gradient(#FFAB00, ${theme.palette.success.main}, ${theme.palette.warning.main})`, animation: 'rotateBorder 4s linear infinite','@keyframes rotateBorder': { '0%': { transform: 'rotate(0deg)' }, '100%': { transform: 'rotate(360deg)' },},}}>
          <Box sx={{ width: '100%', height: '100%', borderRadius: '50%', backgroundColor: 'white.main', }} /></Box>
      ...
      <Drawer anchor="right" open={open} onClose={toggleDrawer(false)}  ModalProps={{ BackdropProps: { sx: { backgroundColor: 'transparent'},},}}>
        <Box sx={{  width: 320 , height: '100vh', position: 'relative', pb: 10 ,}} role="presentation">
          ...
          <Stack spacing={4} alignItems="center" mb={3}>
            ...
            <Typography variant="body2" color="text.secondary">demo@minimals.cc</Typography>
            </Box>
          </Stack>
          ...
          <List sx={{ position: 'relative'}}>
            <ListItem disablePadding>
              <ListItemButton  sx={{ borderRadius: 2, '&:hover': { backgroundColor: 'neutral.light' },}}>
                <Stack direction="row" justifyContent="center">
                 <Box component="img" src={Images.Homeicon} sx={{ width: 25, height: 25, marginRight: 1 }} />
                <ListItemText primary="Home" primaryTypographyProps={{ fontSize: 15, fontWeight: 600, color: 'neutral.main',}}/>
               </Stack>
              </ListItemButton>
            </ListItem>
            <ListItem disablePadding>
              <ListItemButton  sx={{ borderRadius: 2, '&:hover': { backgroundColor: 'neutral.light' },}}>
                ...
                <ListItemText primary="Profile" primaryTypographyProps={{ fontSize: 15, fontWeight: 600, color: 'neutral.main',}}/>
               </Stack>
              </ListItemButton>
            </ListItem>
            <ListItem disablePadding>
              <ListItemButton  sx={{ borderRadius: 2, '&:hover': { backgroundColor: 'neutral.light' },}}>
                ...
                <ListItemText primary="Products" primaryTypographyProps={{ fontSize: 15, fontWeight: 600, color: 'neutral.main',}}/>
                 <Chip label={3}  size="small" sx={{ borderRadius: 1, fontWeight: 700 , ml: 1 ,color: 'red.dark' , backgroundColor: 'red.extralight' }} />
               </Stack>
              </ListItemButton>
            </ListItem>
          </List>
          <Box sx={{ position: 'absolute', bottom: 20, left: 20, right: 20,}}>
           <Button  sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700, color: 'red.dark', backgroundColor: 'orange.light', '&:hover': { backgroundColor: 'red.light' }, py: 1.3, }} fullWidth variant="contained" size="small" >Logout</Button>
        </Box>
        </Box>
      </Drawer>
    </>
